...
@NamedQueries({
        ...
        @NamedQuery(name=Meal.GET_BETWEEN, query = "SELECT m FROM Meal m WHERE m.user.id=:userId " +
                "AND m.dateTime>=:dateFrom AND m.dateTime<:dateTo ORDER BY m.dateTime DESC"),
        @NamedQuery(name=Meal.DELETE, query = "DELETE FROM Meal m WHERE m.id=:id AND m.user.id=:userId"),
        @NamedQuery(name=Meal.UPDATE, query = "UPDATE Meal m SET m.description=:description," +
                "m.calories=:calories, m.dateTime=:dateTime " +
...
@Entity
@Table(name="meals",
        uniqueConstraints = {@UniqueConstraint(columnNames = {"user_id", "date_time"}, name = "uk_user_meal_date_time")})
